...
function getTypeFromMessage(msg) {
  for (let type of Object.keys(typesToLevels)) {
    if (msg.startsWith(`${type}:`) || msg.startsWith(`${type}(`)) return type
  }
  return null
}
await updateRemote()
...
let [major, minor, patch] = lastVersion.split(".").map(s => parseInt(s))
if (maxLevel === 0) {
  patch += 1
} else if (maxLevel === 1) {
  patch = 0
